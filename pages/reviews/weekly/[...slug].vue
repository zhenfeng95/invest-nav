...
import { formatWeekOfMonth } from '~/utils/format'
...
const { data, error, pending, status } = await useAsyncData(
  () => `review-weekly-${slug.value}`,
  () => $fetch<ReportDetail>(`/api/reviews/weekly/${slug.value}`),
  {
    watch: [slug],
...
usePageSeo({
  title: review.value?.title || '每周复盘',
  description: review.value
    ? `${review.value.title}。投研 Agent 周度交易复盘，整理盈亏、纪律与改进要点，仅供学习交流，不构成投资建议。`
    : '投研 Agent 周度交易复盘，整理盈亏、纪律与改进要点，仅供学习交流，不构成投资建议。',
  path: `/reviews/weekly/${slug.value}`,
})
</script>

<template>
  <AppContainer class="py-12 sm:py-16">
    <NuxtLink to="/reviews/weekly" class="inline-flex items-center gap-1 text-sm text-zinc-500 hover:text-zinc-900 dark:hover:text-white">
      返回每周复盘
    </NuxtLink>
