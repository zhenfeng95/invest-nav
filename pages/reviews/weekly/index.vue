...
const { data, error, pending, status } = await useAsyncData(
  'reviews-weekly',
  () => $fetch<ReportListResponse>('/api/reviews/weekly'),
  {
...
usePageSeo({
  title: '每周复盘',
  description: '投研 Agent 每周生成的交易复盘，按周阅读盈亏、纪律与下周改进。',
  path: '/reviews/weekly',
})
</script>
...
<template>
  <AppContainer class="space-y-12 py-12 sm:py-16">
    <PageHero
      ...
      title="每周复盘"
      description="读取投研 Agent 仓库中的 Markdown 周报复盘，归档展示当周盈亏、纪律问题与改进清单。内容仅供学习整理，不构成投资建议。"
    />
    ...
    <EmptyState
      ...
      eyebrow="Coming Soon"
      title="每周复盘即将接入"
      description="配置投研 Agent 所在的 GitHub 仓库后，每周复盘会自动出现在这里。"
    />
    ...
    <EmptyState
      ...
      eyebrow="Empty"
      title="还没有复盘文件"
      description="仓库已连接，但每周复盘目录里还没有可展示的 Markdown 文件。"
    />
